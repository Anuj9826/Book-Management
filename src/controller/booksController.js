...
const createBook = async function (req, res) {
    try {
        ...
        //objectId valid or not and its lenght
        let userdata = req.body.userId
            if (!(/^[0-9a-fA-F]{24}$/.test(userdata))) 
                {return res.status(400).send({ status: false, message: "userId format isn't correct" })}
    ...
    try {
        ...
        if (req.query.userId) {
            ...
            let userIdCheck = await userModel.findById({ _id: userId1 })
                if (!userIdCheck) 
                    { return res.status(404).send({ status: false, msg: "userId is not exist" }) }
