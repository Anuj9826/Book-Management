...
const createUser = async function (req, res) {
    try {
        ...
        if (!(/^(?:(?:\+|0{0,2})91(\s*|[\-])?|[0]?)?([6789]\d{2}([ -]?)\d{3}([ -]?)\d{4})$/.test(data.phone))) { return res.status(400).send({ status: false, message: "Please enter a valid Mobile Number" }) };

        const phoneNumberCheck = await userModel.findOne({ phone: data.phone });
        if (phoneNumberCheck) { return res.status(400).send({ status: false, message: "Phone number is already registered" }) };

        if (!data.email || data.email.trim().length == 0) { return res.status(400).send({ status: false, message: "Email field is required" }) };

        if (!(/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/.test(data.email))) { return res.status(400).send({ status: false, message: "Please provide a valid Email" }) };

        const isEmailAlreadyUsed = await userModel.findOne({ email: data.email });
        if (isEmailAlreadyUsed) { return res.status(400).send({ status: false, message: "Email is already registered" }) };

        if (!data.password || data.password.trim().length == 0) { return res.status(400).send({ status: false, message: "Password is required" }) };
